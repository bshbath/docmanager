...
# Character replacement dictionary
    ...
    def build_folder_file_structure(self, path):
        """
    ...
    async def list_files_in_directory_with_keyword_search(self, projects_folder, directory, search_term, project_name):
        ...
        folder_file_structure = self.build_folder_file_structure(output_directory)
...
def highlight_text_in_pdf(pdf_path, search_term, pages_with_occurrences, output_directory, error_log_path):
    try:
        # Open the original PDF
        doc = fitz.open(pdf_path)
...
def build_folder_structure_for_search_result(self, search_term, project_name):
    ...
    folder_structure = build_folder_structure(output_directory)
    folder_structure["name"] = "root"
    folder_structure["toggled"] = True
    root_folder_id = folder_structure['id']
...
async def list_files_in_directory_with_keyword_search(projects_folder, directory, search_term, project_name):
    projects_directory = projects_folder
    ...
    def process_pdf_file(file_path, search_term):
        ...
        try:
            ...
            if found_keyword:
                # Compute relative path and create corresponding output subdirectory
                relative_path = os.path.relpath(file_path, directory)
                output_subdir = os.path.join(output_directory, replace_german_characters(os.path.dirname(relative_path), 'replaced_german_chars.txt'))
                
                os.makedirs(output_subdir, exist_ok=True)
                highlighted_pdf_path = highlight_text_in_pdf(file_path, search_term, pages_with_occurrences, output_subdir, error_log_path)
                file_name = os.path.basename(highlighted_pdf_path)
    ...
    folder_structure["toggled"] = False
    folder_file_structure = build_folder_file_structure(output_directory)
    return folder_structure, folder_file_structure, occurences_json