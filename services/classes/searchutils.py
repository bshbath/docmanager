...
# Character replacement dictionary
    ...
    def replace_german_characters(self, text, log_file=None):
        ...
        """
        original_text = text
        for german_char, english_rep in self.char_replacements.items():
    ...
    async def list_files_in_directory_with_keyword_search(self, projects_folder, directory, search_term, project_name):
        ...
        skipped_files_log_path = os.path.join(projects_directory, project_name, "SkippedFiles.txt") # TODO: Convert to JSON
        ...
        def process_pdf_file(file_path, search_term):
            ...
            try:
                ...
                if found_keyword:
                    ...
                    directory = self.replace_german_characters(os.path.join(projects_directory, project_name))
        ...
        project_directory = self.replace_german_characters(os.path.join(projects_directory, project_name))
        folder_structure = self.build_folder_structure(project_directory, project_name)
        # folder_structure["name"] = "root" # This should probably be project name

        folder_structure["toggled"] = False
        directory_ = self.replace_german_characters(os.path.join(projects_directory, project_name))
        folder_file_structure = self.build_folder_file_structure(directory_, project_name)
